...
```{r wordcloud BOM}
corpus_wc_bom <- Corpus(VectorSource((wc_db_clean %>%
                                          filter(review_score > 3))[ ,2])) %>%
...
```{r wordcloud NAO BOM}
corpus_wc_NBOM <- Corpus(VectorSource((wc_db_clean %>%
                                          filter(review_score <= 3))[ ,2])) %>%
  tm_map(removeWords, stopwords("portuguese")) %>%
  tm_map(removePunctuation) %>%
  tm_map(removeNumbers) %>%
...
for (i in 1:1200) {
  ...
  #CRIACAO DA VARIAVEL RESPOSTA (Y)
  df$Y5[i] <- if_else(df$review_score[i] == 5, 0, 0)
  df$Y4[i] <- if_else(df$review_score[i] == 4, 0, 0)
  df$Y3[i] <- if_else(df$review_score[i] == 3, 1, 0)
  df$Y2[i] <- if_else(df$review_score[i] == 2, 1, 0)
  df$Y1[i] <- if_else(df$review_score[i] == 1, 1, 0)
  
  df$Y[i] <- df$Y5[i] + df$Y4[i] + df$Y3[i] + df$Y2[i] + df$Y1[i]
  
  #CRIACAO DAS VARIAVEIS DEUMMIES
  df$X_recomendo[i] <- ifelse(str_detect(df$review_comment_message[i], "recomendo"), 1, 0)
  df$X_antes[i] <- ifelse(str_detect(df$review_comment_message[i], "porém"), 1, 0)
  df$X_otimo[i] <- ifelse(str_detect(df$review_comment_message[i], "aguardando"), 1, 0)
  df$X_chegou[i] <- ifelse(str_detect(df$review_comment_message[i], "quero"), 1, 0)
  df$X_adorei[i] <- ifelse(str_detect(df$review_comment_message[i], "apenas"), 1, 0)
  df$X_super[i] <- ifelse(str_detect(df$review_comment_message[i], "estou"), 1, 0)
  df$X_qualidade[i] <- ifelse(str_detect(df$review_comment_message[i], "comprei"), 1, 0)
  df$X_perfeito[i] <- ifelse(str_detect(df$review_comment_message[i], "bom"), 1, 0)
  df$X_prazo[i] <- ifelse(str_detect(df$review_comment_message[i], "ainda"), 1, 0)
  df$X_excelente[i] <- ifelse(str_detect(df$review_comment_message[i], "dias"), 1, 0)
  df$X_recebi[i] <- ifelse(str_detect(df$review_comment_message[i], "não"), 1, 0)
  df$X_rapido[i] <- ifelse(str_detect(df$review_comment_message[i], "outro"), 1, 0)
  df$X_gostei[i] <- ifelse(str_detect(df$review_comment_message[i], "contato"), 1, 0)
  df$X_entrega[i] <- ifelse(str_detect(df$review_comment_message[i], "pedido"), 1, 0)
  df$X_bom[i] <- ifelse(str_detect(df$review_comment_message[i], "site"), 1, 0)
  df$X_bem[i] <- ifelse(str_detect(df$review_comment_message[i], "nada"), 1, 0)
  df$X_boa[i] <- ifelse(str_detect(df$review_comment_message[i], "recebi"), 1, 0)
  df$X_muito[i] <- ifelse(str_detect(df$review_comment_message[i], "qualidade"), 1, 0)
  df$X_sempre[i] <- ifelse(str_detect(df$review_comment_message[i], "correios"), 1, 0)
