...
# db_model_bom <- db_test_log_bom %>% select(c("Y","pred","pred.class"))
...
db_test_rpt_bom <- db_test_bom

db_test_rpt_bom$pred <- predict(rpt_fit_bom, newdata = db_test_rpt_bom)
db_test_rpt_bom$pred.class <- ifelse(db_test_rpt_bom$pred > 0.5,1,0)

# db_model_bom <- db_test_rpt_bom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO

# MATRIZ DE CONFUSAO E METRICAS
ctable_rpt_bom <- table(as.numeric(db_test_rpt_bom$pred.class), db_test_rpt_bom$Y)

acc_rpt_bom <- sum(ctable_rpt_bom[1,1], ctable_rpt_bom[2,2]) / sum(ctable_rpt_bom)

esp_rpt_bom <- ctable_rpt_bom[1,1] / sum(ctable_rpt_bom[,1])

sen_rpt_bom <- ctable_rpt_bom[2,2] / sum(ctable_rpt_bom[,2])
```
...
# db_model_bom <- db_test_rnf_bom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO
...
# db_model_bom <- db_test_xgb_bom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO
...
# CRIACAO DAS VARIAVEIS DUMMIES
  ...
  db_dummy_nbom$X15[i] <-
    ifelse(str_detect(db_dummy_nbom$comp_comment[i], "site"), 1, 0)
  db_dummy_nbom$X16[i] <-
...
# db_model_nbom <- db_test_log_nbom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO
...
# db_model_nbom <- db_test_rnf_nbom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO
...
# db_model_nbom <- db_test_xgb_nbom %>% select(c("Y","pred","pred.class"))
# ^ NÃO ESTÁ SENDO USADO
...
model_comp_table <- data.frame(Modelo = c("Logístico", "Random Forest", "Logístico", "Random Forest"),
                               Tipo = c("Bom", "Bom", "Bom", "Não bom", "Não bom", "Não bom"),
                               Acurácia = c(acc_log_bom,
                                            acc_rnf_bom,
                                            acc_log_nbom,
                                            acc_rnf_nbom,
                                            acc_xgb_bom,
                                            acc_xgb_nbom),
                               Especificidade = c(esp_log_bom,
                                                  esp_rnf_bom,
                                                  esp_log_nbom,
                                                  esp_rnf_nbom,
                                                  esp_xgb_bom,
                                                  esp_xgb_nbom),
                               Sensibilidade = c(sen_log_bom,
                                                 sen_rnf_bom,
                                                 sen_log_nbom,
                                                 sen_rnf_nbom,
                                                 sen_xgb_bom,
